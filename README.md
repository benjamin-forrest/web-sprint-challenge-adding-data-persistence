...
There is no difference between Relational Databases and SQL.

2. Why do tables need a Primary Key?

Tables need a primary key so that you can identify a row with a unique identifier independent of the data being used.

3. What is the name given to a table column that references the Primary Key on another table?

A table column that references the primary key on another table is called the foreign key.

4. What do we need in order to have a _many to many_ relationship between two tables?

We need more than one record in one table that is related to more than one record in the other table.